...
- **Categorias Ativas**: Categoria **A** (Motocicleta) e Categoria **B** (Automóvel);
...
- **Pagamentos Integrados**: Mercado Pago (PIX e Cartão de Crédito) com split e retenção de plataforma;
...
5. **Segurança de Roles no Cadastro**:
   - Cadastros públicos criam exclusivamente a role `STUDENT`.
   - Nenhuma role privilegiada (`INSTRUCTOR`, `SCHOOL_ADMIN`, `PLATFORM_ADMIN`) pode ser autoatribuída via frontend ou `user_metadata`.
...
2. **Horizonte de Agendamento**:
   - `STUDENT_BOOKING_HORIZON_DAYS = 60` é a única constante canônica em [`src/domain/availability.ts`](file:///d:/mazzi_premium_ui_v2/src/domain/availability.ts).
...
3. **Transição de Estados de Booking**:
   - `DRAFT` → `PENDING_PAYMENT` → `CONFIRMED` → `IN_PROGRESS` → `COMPLETED`.
   - O booking só transiciona para `CONFIRMED` após webhook assinado e verificado do gateway de pagamento.
4. **Idempotência**: Todas as transações financeiras e webhooks utilizam `idempotency_key`.
...
2. **Componentes Obrigatórios**:
   - Botão Primário: [`PrimaryButton`](file:///d:/mazzi_premium_ui_v2/src/components/ui/PrimaryButton.tsx) (amarelo, min-h-[48px]);
   - Botão Secundário: [`SecondaryButton`](file:///d:/mazzi_premium_ui_v2/src/components/ui/SecondaryButton.tsx) (branco com borda suave, min-h-[44px]);
   - Entradas de Texto: [`Input`](file:///d:/mazzi_premium_ui_v2/src/components/ui/Input.tsx);
   - Senha: [`PasswordInput`](file:///d:/mazzi_premium_ui_v2/src/components/ui/Input.tsx) com alternância `Eye`/`EyeOff`;
   - OTP: [`OtpInput`](file:///d:/mazzi_premium_ui_v2/src/components/ui/OtpInput.tsx) (6 dígitos, teclado numérico).
