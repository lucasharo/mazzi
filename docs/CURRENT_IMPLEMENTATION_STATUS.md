...
| Pagamentos | Mercado Pago | `HOMOLOGAÇÃO/DEV` | [`src/apps/student/components/MercadoPagoCardCheckout.tsx`](../src/apps/student/components/MercadoPagoCardCheckout.tsx), Edge Function `process-mercadopago-card-payment`, [`docs/09-payments.md`](./09-payments.md) | Alternável por ambiente; somente cartão, uma parcela e credenciais de teste. Produção, split e payouts reais permanecem desabilitados (`DEC-014`). |
