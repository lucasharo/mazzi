...
| Pagamentos | Stripe | `IMPLEMENTADO / HOMOLOGAÇÃO` | [`src/apps/student/components/StripePaymentCheckout.tsx`](../src/apps/student/components/StripePaymentCheckout.tsx), Edge Functions `create-stripe-payment-intent`, `stripe-webhook` e `process-stripe-refund` | Cartão e Pix usam Payment Element; valores vêm do banco, o webhook assinado confirma a reserva e o Admin solicita estornos idempotentes. Connect/split e payout automático permanecem desabilitados. |
