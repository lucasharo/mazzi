...
const migration = readFileSync(
  resolve(process.cwd(), 'supabase/migrations/20260823050139_task_081_correct_marketplace_fee_split.sql'),
  'utf8',
);
...
describe('Platform fee percentage contract', () => {
  ...
  it('keeps the quote total equal to lesson price plus the calculated fee', () => {
    ...
    expect(migration).toContain('v_platform_fee_cents, v_total_in_cents');
    expect(migration).toContain('v_total_in_cents := v_offering.price_in_cents');
    expect(migration).not.toContain('v_offering.price_in_cents + v_platform_fee_cents');
  });
});
