...
const StudentProfileMigratedScreen: React.FC = () => {
  ...
  return (
    <main className="min-h-screen bg-[var(--mazzi-bg)] px-5 py-8 text-[var(--mazzi-dark)]">
      <div className="mx-auto flex min-h-[70vh] w-full max-w-md flex-col justify-center rounded-3xl border border-[var(--mazzi-border)] bg-white p-6 text-center shadow-xs">
        ...
        <p className="mt-5 text-[11px] font-bold uppercase tracking-[0.12em] text-slate-500">Conta atualizada</p>
        <h1 className="mt-1 text-xl font-extrabold">Seu perfil de aluno está desativado.</h1>
        <p className="mt-3 text-sm leading-relaxed text-slate-600">Use esta mesma conta no MAZZI PRO para continuar sua jornada profissional. Seu histórico foi preservado.</p>
...
const StudentGate: React.FC = () => {
  ...
  if (auth.user?.roles.includes('INSTRUCTOR')) return <StudentProfileMigratedScreen />;
  return <AccessDenied />;
};
