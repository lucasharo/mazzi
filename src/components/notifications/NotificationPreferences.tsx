...
export const NotificationPreferences: React.FC<NotificationPreferencesProps> = ({ appContext, showHeading = true }) => {
  ...
  return (
    ...
    >
      ...
      {error && (
        <div className="mt-3 flex items-start justify-between gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-3 text-xs font-semibold text-rose-800" role="alert">
          <span>{error}</span>
        ...
        {isLoading ? (
          <div className="space-y-2" aria-label="Carregando preferências…">
            ...
            {definitions.slice(0, 4).map((definition) => (
              <div key={definition.type} className="h-16 animate-pulse rounded-2xl bg-slate-100 motion-reduce:animate-none" aria-hidden="true" />
            ))}
          </div>
        ) : error ? (
