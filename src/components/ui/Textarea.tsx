...
export const Textarea: React.FC<TextareaProps> = ({
  ...
  return (
    <div className="w-full space-y-1.5 text-left">
      {label && <label htmlFor={fieldId} className="mazzi-field-label block">{label}</label>}
      <textarea
        id={fieldId}
        aria-invalid={error ? true : undefined}
        aria-describedby={error || helperText ? messageId : undefined}
        className={`w-full resize-none rounded-2xl border bg-white px-3.5 py-2.5 text-sm text-[var(--mazzi-text)] outline-none transition focus:border-[var(--mazzi-yellow)] focus:ring-2 focus:ring-[var(--mazzi-focus-glow)] disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400 ${error ? 'border-rose-500' : 'border-[var(--mazzi-border)]'} ${className}`}
        {...props}
      />
