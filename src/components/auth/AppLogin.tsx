...
const fieldClass = 'mazzi-input mt-2';
...
export const AppLogin: React.FC<{ kind: AppLoginKind }> = ({ kind }) => {
  ...
  const shell = (content: React.ReactNode) => <main className={`min-h-[100dvh] w-full bg-[var(--mazzi-bg)] ${kind === 'admin' ? 'grid place-items-center p-5' : ''}`}><div className={kind === 'admin' ? 'mazzi-card w-full max-w-md p-8' : 'mx-auto flex min-h-[100dvh] w-full max-w-lg flex-col justify-center px-6 py-12'}>{content}</div></main>;
  ...
  const headline = kind === 'student' ? <>Aprenda.<br/><span className="text-[var(--mazzi-yellow)]">Dirija.</span><br/>Conquiste.</> : kind === 'instructor' ? <>Sua agenda.<br/><span className="text-[var(--mazzi-yellow)]">Suas aulas.</span><br/>Seu negócio.</> : 'Operação MAZZI';
  return shell(<form onSubmit={submitLogin} className="space-y-5"><div className="mb-10"><p className="mazzi-eyebrow mb-5">{kind === 'instructor' ? 'MAZZI PRO' : kind === 'admin' ? 'MAZZI ADMIN' : 'MAZZI'}</p><h1 className="text-[42px] font-extrabold leading-[.98] tracking-[-.055em] text-[var(--mazzi-dark)]">{headline}</h1><p className="mt-4 max-w-xs text-sm leading-6 text-[var(--mazzi-muted)]">{kind === 'student' ? 'Encontre profissionais verificados e marque sua próxima aula prática.' : kind === 'instructor' ? 'Organize sua rotina e acompanhe seus alunos em um só lugar.' : 'Acesso seguro ao painel administrativo.'}</p></div>{field('E-mail', email, setEmail, 'email', 'email')}{field('Senha', password, setPassword, 'password', 'current-password')}{error && <p role="alert" className="rounded-2xl bg-rose-50 p-3 text-sm text-rose-700">{error}</p>}{notice}<button type="submit" disabled={isLoading} className="mazzi-primary w-full disabled:opacity-50">{isLoading ? 'Entrando…' : 'Entrar'}</button><div className="flex items-center justify-between gap-3"><button type="button" onClick={() => goTo('forgot')} className={linkClass}>Esqueceu sua senha?</button>{kind === 'student' && <button type="button" onClick={() => goTo('signup')} className={linkClass}>Criar conta</button>}{kind === 'instructor' && <button type="button" onClick={() => goTo('signup')} className={linkClass}>Quero ser instrutor</button>}</div>{DevQuickLogin && <DevQuickLogin kind={kind} onError={(message) => setFeedback({ tone: 'error', message })}/>}</form>);
};
