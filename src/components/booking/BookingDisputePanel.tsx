  ...
  if (activeDispute) {
    ...
    const contestContent = (
      <section className="mazzi-compact-card space-y-3 rounded-2xl border border-amber-200 bg-amber-50 p-4" aria-label="Contestação da reserva">
        ...
        {activeDispute.informationRequest && <p className="rounded-2xl border border-amber-200 bg-white p-3 text-xs font-semibold text-amber-900"><strong>Informações solicitadas:</strong> {activeDispute.informationRequest}</p>}
        {activeDispute.messages && activeDispute.messages.length > 0 ? <div className="space-y-2" aria-label="Mensagens da contestação"><p className="text-[10px] font-black uppercase text-slate-500">Mensagens da contestação</p>{activeDispute.messages.map((message) => <div key={message.id} className="rounded-2xl bg-white/80 p-3"><div className="mb-1 flex items-center justify-between gap-2"><p className="text-[10px] font-black uppercase text-slate-400">{message.type === 'DESCRIPTION' ? 'Relato da contestação' : message.type === 'INFORMATION_REQUEST' ? 'Solicitação do Admin' : 'Resposta'}</p><time className="text-[10px] font-semibold text-slate-400">{new Date(message.createdAt).toLocaleString('pt-BR')}</time></div><p className="mb-1 text-[11px] font-bold text-slate-600">{message.authorRole === 'ADMIN' ? 'Admin' : message.authorRole === 'STUDENT' ? 'Aluno' : 'PRO'}</p><p className="text-xs text-slate-700">{message.content}</p></div>)}</div> : <p className="rounded-2xl bg-white/80 p-3 text-xs text-slate-700">{activeDispute.description}</p>}
        {(isLoadingEvidence || evidence.length > 0 || evidenceError) && <div className="space-y-2 rounded-2xl border border-amber-200 bg-white/60 p-3"><p className="text-[10px] font-black uppercase text-slate-500">Arquivos enviados</p>{isLoadingEvidence && <p className="text-xs font-semibold text-slate-500">Carregando arquivos...</p>}{evidence.length > 0 && <ul className="space-y-2">{evidence.map((item) => <li key={item.id}><ButtonBase onClick={() => void openEvidence(item)} className="flex min-h-11 w-full items-center gap-2 rounded-2xl border border-amber-100 bg-white p-3 text-left"><FileText className="h-4 w-4 shrink-0 text-amber-700" aria-hidden="true" /><span className="min-w-0 flex-1 truncate text-xs font-bold text-slate-700">{item.originalName}</span><span className="text-[10px] font-semibold text-slate-500">{item.uploadedBy === currentUserId ? 'Você' : 'Outra parte'}</span><ExternalLink className="h-3.5 w-3.5 shrink-0 text-slate-400" aria-hidden="true" /></ButtonBase></li>)}</ul>}{evidenceError && <p role="alert" className="text-xs font-bold text-rose-700">{evidenceError}</p>}</div>}
    ...
    <>
      ...
      >
        <div className="space-y-6 pb-4">
          ...
          {error && <p role="alert" className="rounded-2xl border border-rose-200 bg-rose-50 p-3 text-sm font-bold text-rose-700">{error}</p>}
        </div>
      </Modal>
    </>
  );
