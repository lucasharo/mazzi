...
export const ProviderAddressForm: React.FC<Props> = ({ value, onChange, idPrefix }) => {
  ...
  return <fieldset className="space-y-3 rounded-2xl border border-[var(--mazzi-border)] bg-slate-50/60 p-4">
    ...
    {!isMapPin && <div className="grid grid-cols-[minmax(0,1fr)_7rem] gap-3">
      <div>
        <label className="mazzi-field-label mb-1.5 block" htmlFor={`${idPrefix}-street`}>Logradouro *</label>
        {manualSearch ? <ConfirmableAddressAutocomplete
          id={`${idPrefix}-street`}
          value={value.addressLine1}
          ariaLabel="Buscar endereço manualmente"
          onChange={(addressLine1) => onChange({ ...value, addressLine1, address: undefined })}
        ...
        /> : <Input
          id={`${idPrefix}-street`}
          value={value.addressLine1}
          readOnly={normalizePostalCode(value.postalCode).length === 8}
          onChange={(event) => onChange({ ...value, addressLine1: event.target.value, address: undefined })}
    ...
    {value.address && validation.valid ? <div className="flex items-center gap-2 rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-xs font-semibold text-emerald-800"><CheckCircle2 className="h-4 w-4 shrink-0" /> {isMapPin ? 'Localização confirmada no mapa' : isNoNumber ? 'Endereço da rua localizado' : `Endereço localizado: ${value.address.formatted || value.address.addressLine1 || 'endereço confirmado'}`}</div> : !manualSearch && <ButtonBase type="button" className="text-left text-xs font-bold text-[var(--mazzi-dark)] underline" onClick={() => setManualSearch(true)}>Buscar endereço manualmente</ButtonBase>}
