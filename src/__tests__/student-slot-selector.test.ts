...
describe('Student Experience phases 3 and 4', () => {
  ...
  it('preserves the configured 90-day horizon', () => {
    expect(INITIAL_WINDOW_DAYS).toBe(90);
    expect(LOAD_MORE_DAYS).toBe(30);
    expect(MAX_HORIZON_DAYS).toBe(90);
    expect(Math.min(MAX_HORIZON_DAYS, INITIAL_WINDOW_DAYS + LOAD_MORE_DAYS)).toBe(90);
  });
