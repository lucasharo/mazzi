...
import type { Booking, InstantLessonInstructorStatus, InstantLessonOffer, InstantLessonPlatformConfig, InstantLessonSettings, Provider, ServiceOffering, Vehicle } from '../../../types';
...
interface ProviderInstantLessonPanelProps {
  ...
  platformConfig: InstantLessonPlatformConfig;
...
export const ProviderInstantLessonPanel: React.FC<ProviderInstantLessonPanelProps> = ({ provider, offerings, vehicles, instructorOptions, availabilityInstructorOptions = instructorOptions, settings, platformConfig, instructorStatuses, currentUserId, canManageInstructorAvailability = false, onSave, onToggleOnline, isLoading, offers, pendingPaymentInstantBookings, instantOffersServerNow, onRespondOffer, offerAction }) => {
  ...
  const save = async (configuration: (typeof configurations)[number], enabledOverride?: boolean) => {
    const draft = { ...getDraft(configuration), ...(enabledOverride === undefined ? {} : { enabled: enabledOverride }) };
    let price: number;
    try {
    ...
    <section className="space-y-4" aria-labelledby="instant-lesson-title">
      <div className="mazzi-compact-card rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-white p-3 shadow-sm ring-1 ring-amber-100/70">
        <div className="flex items-start gap-2.5">
          ...
          <div><h2 id="instant-lesson-title" className="text-sm font-extrabold text-[var(--mazzi-dark)]">Aula Agora</h2><p className="mt-0.5 text-xs font-medium leading-snug text-slate-600">Defina quando você aceita aulas imediatas, o valor do seu trabalho e até onde pode se deslocar.</p></div>
        </div>
      <div className="mt-3 flex flex-wrap items-center justify-center gap-2"><Badge variant="neutral">Deslocamento máximo {platformConfig.maxEtaMinutes} min</Badge><Badge variant="neutral">A oferta expira em {platformConfig.offerExpirationSeconds}s</Badge></div>
      </div>
      <div className="flex items-center gap-1.5 px-1 text-[11px] font-semibold text-emerald-700" role="status" aria-live="polite"><Radio className="h-3.5 w-3.5" aria-hidden="true" /> <span>Atualização em tempo real</span></div>
       <section className="space-y-2" aria-label="Disponibilidade do instrutor">{availabilityInstructorOptions.map((instructor) => { const instructorStatus = getInstructorStatus(instructor.id); const isOnline = instructorStatus ? isInstantInstructorAvailabilityActive(instructorStatus, now + serverClockOffsetMs) : false; const hasExpired = Boolean(instructorStatus?.instantOnline && instructorStatus.onlineExpiresAt && !isOnline); const availabilityText = instructorStatus ? formatInstantInstructorAvailability(instructorStatus, now + serverClockOffsetMs) : null; const enabledVehicleCount = settings.some((setting) => setting.instructorId === instructor.id && setting.instantEnabled); const canToggle = currentUserId === instructor.id || canManageInstructorAvailability; const handleToggle = (online: boolean) => { void onToggleOnline(instructor.id, online).catch(() => setError('Não foi possível atualizar a disponibilidade deste instrutor.')); }; return <div key={instructor.id} className="mazzi-compact-card flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-[var(--mazzi-dark)] p-4 shadow-sm"><div className="min-w-0"><p className="text-sm font-semibold text-white">{instructor.name}</p><p className="mt-0.5 text-xs font-normal text-white/75">{hasExpired ? 'A disponibilidade da Aula Agora expirou. Ative novamente para receber novas solicitações.' : availabilityText || (canToggle ? 'Disponível por até 1h após a ativação.' : 'A disponibilidade da Aula Agora é controlada pelo instrutor ou pela autoescola.')} {!enabledVehicleCount && ' Habilite pelo menos um carro para ficar disponível.'}</p></div>{canToggle ? <label className={`inline-flex min-h-11 shrink-0 items-center gap-2 text-sm font-semibold ${enabledVehicleCount && !isLoading ? 'cursor-pointer text-white' : 'cursor-not-allowed text-white/50'}`}><input type="checkbox" className="peer sr-only" checked={isOnline} disabled={!enabledVehicleCount || isLoading} onChange={(event) => handleToggle(event.target.checked)} /><span aria-hidden="true" className="relative h-6 w-11 shrink-0 rounded-full bg-white/20 shadow-inner transition-colors after:absolute after:left-1 after:top-1 after:h-4 after:w-4 after:rounded-full after:bg-white after:shadow-sm after:transition-transform peer-checked:bg-[var(--mazzi-yellow)] peer-checked:after:translate-x-5 peer-focus-visible:ring-2 peer-focus-visible:ring-[var(--mazzi-yellow)] peer-focus-visible:ring-offset-2 peer-focus-visible:ring-offset-[var(--mazzi-dark)] peer-disabled:opacity-50" /><span className="whitespace-nowrap">{isOnline ? 'Online' : 'Offline'}</span></label> : <Badge variant={isOnline ? 'success' : 'neutral'}>{isOnline ? 'Disponível' : 'Offline'}</Badge>}</div>; })}</section>
      ...
      {configurations.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center"><Car className="mx-auto h-8 w-8 text-slate-400" aria-hidden="true" /><p className="mt-3 font-extrabold text-slate-800">Nenhum veículo ou instrutor disponível</p><p className="mt-1 text-sm text-slate-500">Ative um veículo e um instrutor elegível para configurar a Aula Agora. Isso não altera a agenda.</p></div> : configurations.map((configuration) => {
        ...
        return <article key={configuration.id} className="mazzi-compact-card rounded-2xl border border-[var(--mazzi-border)] bg-white p-4 shadow-sm">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2"><Input label="Preço da Aula Agora" value={draft.price} onChange={(event) => updateDraft(configuration, { price: maskBRLInput(event.target.value) })} onBlur={() => void save(configuration)} disabled={draft.enabled || savingId === configuration.id} inputMode="decimal" placeholder="R$ 0,00" /><Select label="Distância máxima" value={String(draft.distance)} options={distanceOptions} onChange={(event) => updateDraft(configuration, { distance: Number(event.target.value) })} onBlur={() => void save(configuration)} disabled={draft.enabled || savingId === configuration.id} /></div>
          <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50/70 p-3"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="text-sm font-extrabold text-[var(--mazzi-dark)]">Carro habilitado para Aula Agora</p><p className="mt-0.5 text-xs font-medium text-slate-600">{draft.enabled ? 'Desative para editar preço e distância.' : 'Edite os dados e ative quando estiver pronto.'}</p></div><label aria-label={`${draft.enabled ? 'Desativar' : 'Ativar'} este carro para Aula Agora`} className={`inline-flex min-h-11 shrink-0 items-center gap-2 text-sm font-extrabold ${hasPrice && savingId !== configuration.id ? 'cursor-pointer text-slate-700' : 'cursor-not-allowed text-slate-400'}`}><input type="checkbox" className="peer sr-only" checked={draft.enabled} disabled={!hasPrice || savingId === configuration.id} onChange={(event) => { updateDraft(configuration, { enabled: event.target.checked }); void save(configuration, event.target.checked); }} /><span aria-hidden="true" className="relative h-6 w-11 shrink-0 rounded-full bg-slate-300 shadow-inner transition-colors after:absolute after:left-1 after:top-1 after:h-4 after:w-4 after:rounded-full after:bg-white after:shadow-sm after:transition-transform peer-checked:bg-[var(--mazzi-yellow)] peer-checked:after:translate-x-5 peer-focus-visible:ring-2 peer-focus-visible:ring-[var(--mazzi-yellow)] peer-focus-visible:ring-offset-2 peer-disabled:opacity-50" /><span className="sr-only">{draft.enabled ? 'Ativo' : 'Desativado'}</span></label></div></div>
        </article>;
      })}
    </section>
  );
