...
export const ProviderInstantLessonPanel: React.FC<ProviderInstantLessonPanelProps> = ({ provider, offerings, vehicles, instructorOptions, settings, onSave, onToggleOnline, isLoading, offers, pendingPaymentInstantBookings, instantOffersServerNow, onRespondOffer, offerAction }) => {
  ...
  const enabledVehicleCount = settings.filter((setting) => setting.instantEnabled).length;
  const isInstructorOnline = settings.some((setting) => setting.instantEnabled && setting.instantOnline);
  useEffect(() => {
  ...
  return (
    <section className="space-y-4" aria-labelledby="instant-lesson-title">
      ...
      <div className="mazzi-compact-card flex items-center justify-between gap-3 rounded-2xl border border-amber-200 bg-white p-4 shadow-sm"><div className="min-w-0"><p className="text-sm font-extrabold text-[var(--mazzi-dark)]">Disponibilidade do instrutor</p><p className="mt-0.5 text-xs font-medium text-slate-600">Um único controle para aceitar Aula Agora. Os carros habilitados continuam independentes.</p></div><label className={`inline-flex min-h-11 shrink-0 items-center gap-2 text-sm font-extrabold ${enabledVehicleCount > 0 && !isLoading ? 'cursor-pointer text-slate-700' : 'cursor-not-allowed text-slate-400'}`}><input type="checkbox" className="peer sr-only" checked={isInstructorOnline} disabled={enabledVehicleCount === 0 || isLoading} onChange={(event) => void onToggleOnline(event.target.checked)} /><span aria-hidden="true" className="relative h-6 w-11 shrink-0 rounded-full bg-slate-300 shadow-inner transition-colors after:absolute after:left-1 after:top-1 after:h-4 after:w-4 after:rounded-full after:bg-white after:shadow-sm after:transition-transform peer-checked:bg-[var(--mazzi-yellow)] peer-checked:after:translate-x-5 peer-focus-visible:ring-2 peer-focus-visible:ring-[var(--mazzi-yellow)] peer-focus-visible:ring-offset-2 peer-disabled:opacity-50" /><span className="whitespace-nowrap">Aceitar Aula Agora</span></label></div>
      ...
      {configurations.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center"><Car className="mx-auto h-8 w-8 text-slate-400" aria-hidden="true" /><p className="mt-3 font-extrabold text-slate-800">Nenhum veículo ou instrutor disponível</p><p className="mt-1 text-sm text-slate-500">Ative um veículo e um instrutor elegível para configurar a Aula Agora. Isso não altera a agenda.</p></div> : configurations.map((configuration) => {
        const draft = getDraft(configuration); const setting = settings.find((item) => item.instructorId === configuration.instructorId && item.vehicleId === configuration.vehicleId); const vehicle = vehicles.find((item) => item.id === configuration.vehicleId);
