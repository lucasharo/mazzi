    ...
    <div className="space-y-6 text-left">
      ...
      <Modal isOpen={isEmergencyModalOpen} onClose={() => !isSavingEmergencyBlock && setIsEmergencyModalOpen(false)} title={emergencyEditingBlockId ? 'Editar bloqueio rápido' : 'Bloqueio rápido'}>
        <div className="space-y-4 text-left">
          ...
          {emergencySelectedSlots.length > 0 && <div className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 p-3 text-xs font-semibold text-slate-700"><span>{emergencySelectedSlots[0].startTime} às {emergencySelectedSlots[emergencySelectedSlots.length - 1].endTime} · {emergencySelectedSlots.length} {emergencySelectedSlots.length === 1 ? 'hora selecionada' : 'horas selecionadas'}</span><Button type="button" variant="ghost" size="sm" className="shrink-0 text-slate-600" onClick={() => setEmergencySelectedSlots([])}>Alterar</Button></div>}
