...
CREATE INDEX "compliance_documents_terms_history_idx"
  ON "public"."compliance_documents" USING btree ("provider_id", "user_id", "document_type", "terms_version", "accepted_at" DESC)
...
BEGIN
  ...
  WHERE provider_id=p_provider_id AND user_id=auth.uid()
    ...
    AND terms_version=v_current_version AND storage_path=v_path
  ORDER BY accepted_at DESC LIMIT 1;
  IF FOUND THEN RETURN v_doc; END IF;
  INSERT INTO public.compliance_documents(provider_id,user_id,vehicle_id,membership_id,scope,document_type,storage_path,status,terms_version,document_hash,accepted_at)
  VALUES(p_provider_id,auth.uid(),NULL,NULL,'PROVIDER'::public.compliance_document_scope,
    ...
    v_current_version,'sha256:c256a7d94920ada35d9a7ad6528cb38a1bf7cdd23e0e336d6b4c48eba2c8b69f',NOW())
  RETURNING * INTO v_doc;
  INSERT INTO public.audit_logs(actor_id,action,entity_type,entity_id,new_value,severity,created_at)
  VALUES(auth.uid(),'MAZZI_TERMS_ACCEPTED','COMPLIANCE_DOCUMENTS',v_doc.id,
    jsonb_build_object('provider_id',p_provider_id,'document_type','MAZZI_TERMS_ACCEPTANCE','terms_version',v_current_version,'document_hash','sha256:c256a7d94920ada35d9a7ad6528cb38a1bf7cdd23e0e336d6b4c48eba2c8b69f','accepted_at',v_doc.accepted_at,'scope','PROVIDER'),'INFO',NOW());
  RETURN v_doc;
END; $$;
...
-- MAZZI MVP baseline: self-booking and multi-role UX contract (20260825211702)
...
-- Forward-only contract: 20260825223009_geoapify_provider_address
-- Exact coordinates remain private provider fields; public discovery continues
-- to use the existing public_latitude/public_longitude projection.
...
BEGIN
  v_uid := auth.uid();
  IF v_uid IS NULL THEN RAISE EXCEPTION 'UNAUTHORIZED: Usuário não autenticado.' USING ERRCODE = '28000'; END IF;
  IF NOT (public.is_provider_owner(p_provider_id) OR public.is_school_admin(p_provider_id)) THEN RAISE EXCEPTION 'PROVIDER_PROFILE_ACCESS_DENIED: Você não tem permissão para atualizar este perfil de prestador.' USING ERRCODE = '42501'; END IF;
  IF p_name IS NOT NULL THEN v_clean_name := trim(p_name); IF v_clean_name = '' THEN RAISE EXCEPTION 'PROVIDER_NAME_INVALID: O nome do prestador não pode ser vazio.' USING ERRCODE = '22000'; END IF; END IF;
  IF p_public_contact IS NOT NULL THEN v_clean_contact := trim(p_public_contact); IF v_clean_contact <> '' AND NOT (v_clean_contact ~ '^\d{10,11}$') THEN RAISE EXCEPTION 'PROVIDER_CONTACT_INVALID: O contato público deve conter 10 ou 11 dígitos numéricos.' USING ERRCODE = '22000'; END IF; IF v_clean_contact = '' THEN v_clean_contact := NULL; END IF; END IF;
  IF p_neighborhood IS NOT NULL THEN v_clean_neighborhood := NULLIF(trim(p_neighborhood), ''); END IF;
  IF p_city IS NOT NULL THEN v_clean_city := trim(p_city); IF v_clean_city = '' THEN RAISE EXCEPTION 'PROVIDER_CITY_INVALID: A cidade do prestador não pode ser vazia.' USING ERRCODE = '22000'; END IF; END IF;
  IF p_state IS NOT NULL THEN v_clean_state := upper(trim(p_state)); IF v_clean_state = '' OR NOT (v_clean_state ~ '^[A-Z]{2}$') THEN RAISE EXCEPTION 'PROVIDER_STATE_INVALID: O estado (UF) deve ter exatamente 2 letras.' USING ERRCODE = '22000'; END IF; END IF;
  IF p_service_radius_km IS NOT NULL AND (p_service_radius_km < 1 OR p_service_radius_km > 100) THEN RAISE EXCEPTION 'SERVICE_RADIUS_INVALID: O raio de atendimento deve estar entre 1 e 100 km.' USING ERRCODE = '22000'; END IF;
  ...
  IF p_latitude IS NOT NULL AND (p_latitude < -90 OR p_latitude > 90) THEN RAISE EXCEPTION 'PROVIDER_LATITUDE_INVALID' USING ERRCODE = '22000'; END IF;
  IF p_longitude IS NOT NULL AND (p_longitude < -180 OR p_longitude > 180) THEN RAISE EXCEPTION 'PROVIDER_LONGITUDE_INVALID' USING ERRCODE = '22000'; END IF;
  UPDATE public.providers
