-- Keep the public agenda marketplace isolated from internal Aula Agora rows.
-- Aula Agora offerings are booking anchors and must never be returned by the
-- scheduled-lesson search or used to satisfy its date availability filter.
...
BEGIN
  ...
  WITH eligible_offerings AS (
    ...
    WHERE o.source = 'AGENDA'
      AND o.is_active = TRUE AND o.status = 'ACTIVE'
