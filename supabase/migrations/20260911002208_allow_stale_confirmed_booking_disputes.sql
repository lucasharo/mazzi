-- Aulas antigas que permaneceram CONFIRMED precisam poder ser contestadas.
-- O frontend oferece essa ação apenas quando o horário final já passou;
-- o backend mantém a autorização, a janela configurada e a participação na reserva.
...
BEGIN
  ...
  v_reference_at := CASE
    WHEN v_booking.status = 'COMPLETED' THEN COALESCE(v_booking.completed_at, v_booking.lesson_finished_at, v_booking.scheduled_end_at, v_booking.updated_at)
    ELSE COALESCE(v_booking.scheduled_end_at, v_booking.lesson_finished_at, v_booking.updated_at)
  END;
