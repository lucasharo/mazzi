-- MAZZI — final runtime eligibility hardening
-- Forward-only correction for booking updates and public search.
...
BEGIN
  ...
  IF NEW.selection_mode IS NULL THEN
    NEW.selection_mode := 'SPECIFIC_INSTRUCTOR';
  END IF;
...
-- Keep public search aligned with the same canonical eligibility predicate.
...
REVOKE ALL ON FUNCTION public.search_providers_public(DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, TEXT, TEXT, TEXT, DOUBLE PRECISION, INT, INT, INT, DATE) FROM PUBLIC;
