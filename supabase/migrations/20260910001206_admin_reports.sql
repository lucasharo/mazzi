-- ============================================================================
-- MAZZI — Admin operational reports
-- ============================================================================
-- A single privileged read RPC keeps the Admin report surface authoritative,
-- bounded by period, free of PII/secrets, and independent from table RLS.
...
begin
  ...
  revenue_report as (
    select jsonb_build_object(
      'summary', jsonb_build_object(
        'gross_volume_cents', coalesce(sum(amount_in_cents), 0),
        'paid_volume_cents', coalesce(sum(amount_in_cents) filter (where status::text = 'PAID'), 0),
        'gateway_fee_cents', coalesce(sum(gateway_fee_in_cents), 0),
        'platform_fee_cents', coalesce(sum(b.platform_fee_in_cents) filter (where p.status::text = 'PAID'), 0),
        'payments_created', count(*),
        'payments_paid', count(*) filter (where status::text = 'PAID'),
        'payments_failed', count(*) filter (where status::text in ('FAILED', 'CANCELLED', 'EXPIRED'))
      ),
      'rows', coalesce((
