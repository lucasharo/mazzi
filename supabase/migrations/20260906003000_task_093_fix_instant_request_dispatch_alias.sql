-- TASK-093 follow-up — avoid PL/pgSQL record/SQL alias ambiguity in Aula Agora dispatch.
-- DEV only.
...
BEGIN
  ...
  LOOP
    ...
    VALUES(p_request_id,v_candidate.provider_id,v_candidate.offering_id,v_candidate.instructor_id,v_candidate.vehicle_id,v_candidate.instant_price_in_cents,GREATEST(0,v_candidate.distance),GREATEST(0,v_candidate.eta),v_now+MAKE_INTERVAL(secs=>v_offer_expiration_seconds),'instant_offer:'||p_request_id::TEXT||':'||v_candidate.instructor_id::TEXT)
    ...
    IF FOUND THEN
      ...
      SELECT v_candidate.instructor_id,'INSTANT_LESSON_OFFER','Nova Aula Agora','Há uma solicitação de aula próxima para você avaliar.','instant_offer',v_offer_id,'PRO','instant_offer'
      WHERE EXISTS(SELECT 1 FROM public.users u WHERE u.id=v_candidate.instructor_id AND u.status='ACTIVE');
    END IF;
  END LOOP;
