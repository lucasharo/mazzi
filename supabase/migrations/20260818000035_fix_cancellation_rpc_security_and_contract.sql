-- ============================================================================
-- MAZZI MIGRATION 20260818000035: FIX CANCELLATION RPC SECURITY & CONTRACT
-- ============================================================================

-- 1. Remove old 4-argument signature containing p_is_legal_override
DROP FUNCTION IF EXISTS public.cancel_booking_v2(UUID, TEXT, TEXT, BOOLEAN);
DROP FUNCTION IF EXISTS public.cancel_booking_v2(UUID, TEXT, TEXT);

-- 2. Create canonical 3-argument RPC cancel_booking_v2
...
DECLARE
  ...
  v_is_school_member BOOLEAN := FALSE;
...
BEGIN
  ...
  ELSIF v_user_role IN ('INSTRUCTOR', 'SCHOOL_ADMIN', 'SCHOOL_STAFF', 'DRIVING_SCHOOL') THEN
    -- Check if actor is directly the instructor
    IF v_booking.instructor_id = v_actor_id THEN
      -- Direct match on instructor_id (users.id)
      NULL;
    ELSE
      -- Check if actor is provider owner
      ...
      ELSE
        ...
        SELECT EXISTS (
          ...
          WHERE provider_id = v_booking.provider_id 
            ...
            AND status = 'ACTIVE'
        ) INTO v_is_school_member;

        IF NOT v_is_school_member THEN
          RAISE EXCEPTION 'UNAUTHORIZED_PROVIDER: Acesso negado. Você não pertence a este prestador ou escola.' USING ERRCODE = '40302';
        END IF;
      END IF;
    END IF;

    -- Validate Provider reason code whitelist
  ...
  -- 6. Strict Status Whitelist (Only CONFIRMED and PENDING_PAYMENT are cancelable via this RPC)
  IF v_booking.status::text NOT IN ('CONFIRMED', 'PENDING_PAYMENT') THEN
    RAISE EXCEPTION 'INVALID_STATUS: O status atual (%) não permite cancelamento comercial.', v_booking.status USING ERRCODE = '42200';
  END IF;

  -- 7. Compute antecedence from scheduled_start_at (DEC-013)
  ...
  -- 9. Insert Audit Log (ip_address = NULL when unknown)
