...
-- A revisão administrativa deve receber a validade na mesma operação da
-- aprovação. A função legada de três argumentos permitia aprovar sem data.
DROP FUNCTION IF EXISTS public.review_compliance_document(uuid, public.compliance_status, text);
...
BEGIN
  ...
  IF p_status = 'APPROVED'
     ...
     AND v_doc.document_type::text IN (
       'CNH',
       'CNH_EAR',
       'CREDENTIAL_DETRAN',
    ...
    RAISE EXCEPTION 'EXPIRATION_DATE_REQUIRED'
      USING ERRCODE = '22023',
            HINT = 'Informe a data de validade antes de aprovar este documento.';
  END IF;
...
REVOKE ALL ON FUNCTION public.review_compliance_document(uuid, public.compliance_status, text, timestamptz) FROM PUBLIC, anon;
