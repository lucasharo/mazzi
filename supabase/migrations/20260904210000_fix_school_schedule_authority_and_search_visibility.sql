-- Corrige a autoridade do gestor da autoescola e a elegibilidade pública
-- de ofertas sem regra semanal recorrente ativa.
    ...
    AND (
      ...
      OR (
        EXISTS (
          ...
          WHERE p.id = target_provider_id
            AND p.user_id = auth.uid()
            AND p.type = 'DRIVING_SCHOOL'::public.provider_type
        )
