-- Prevent an instructor who declined an Aula Agora offer from receiving
-- another offer from the same student for five minutes.

BEGIN;

CREATE INDEX IF NOT EXISTS idx_instant_offers_decline_cooldown
  ON public.instant_lesson_offers (instructor_id, status, updated_at, request_id);
      ...
      WHERE s.instant_enabled=TRUE AND o.category=v_req.category
        ...
        AND NOT EXISTS(
          ...
          WHERE declined.instructor_id=o.instructor_id
            ...
            AND declined.updated_at>v_now-INTERVAL '5 minutes'
        )
