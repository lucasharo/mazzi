-- MAZZI — allow a school to reactivate a previously suspended instructor.
-- Reuse the existing activation readiness and authorization gates.
...
BEGIN
  ...
  IF v_membership.membership_status NOT IN (
    'PENDING_COMPLIANCE'::public.school_membership_status,
    'SUSPENDED'::public.school_membership_status
  ) THEN
  ...
  SELECT * INTO v_school FROM public.providers WHERE id = v_membership.school_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'SCHOOL_NOT_FOUND'; END IF;
  IF v_school.status <> 'ACTIVE'::public.provider_status THEN RAISE EXCEPTION 'PROVIDER_NOT_ACTIVE'; END IF;
  IF v_school.type <> 'DRIVING_SCHOOL'::public.provider_type THEN RAISE EXCEPTION 'PROVIDER_NOT_DRIVING_SCHOOL'; END IF;

  SELECT * INTO v_user FROM public.users WHERE id = v_membership.user_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'USER_NOT_FOUND'; END IF;
  IF v_user.status <> 'ACTIVE'::public.user_status THEN RAISE EXCEPTION 'USER_NOT_ACTIVE'; END IF;
  IF NOT (
    v_user.role = 'INSTRUCTOR'::public.user_role
    OR EXISTS (
      SELECT 1 FROM public.user_roles ur
      WHERE ur.user_id = v_user.id AND ur.role = 'INSTRUCTOR'::public.user_role
    )
  ) THEN
