-- Expose all distinct payout milestones to the PRO UI. The date field remains
-- the primary display date for backwards compatibility; schedule and Stripe
-- dates are returned separately so the client cannot conflate their meaning.
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'stripe_available_on', po.stripe_available_on,
    ...
    'external_payout_id', po.external_payout_id,
    'stripe_payout_status', po.stripe_payout_status,
    'status', po.status,
  ...
  ) ORDER BY COALESCE(po.stripe_arrival_date, po.stripe_available_on, po.scheduled_release_at)), '[]'::JSONB)
  INTO v_result
