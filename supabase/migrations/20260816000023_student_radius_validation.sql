-- Add server-side distance enforcement to the meeting-point booking RPC.
...
begin
  ...
  if coalesce(p_meeting_point->>'type', '') = 'STUDENT_ADDRESS' then
    if nullif(btrim(p_meeting_point->>'address'), '') is null
       or p_meeting_point->>'latitude' is null or p_meeting_point->>'longitude' is null
    ...
    if not exists (
      ...
      where q.id = p_quote_id and q.student_id = auth.uid() and p.location_geography is not null
        and st_dwithin(p.location_geography, st_setsrid(st_makepoint((p_meeting_point->>'longitude')::double precision, (p_meeting_point->>'latitude')::double precision), 4326)::geography, p.service_radius_km * 1000)
    ) then raise exception 'STUDENT_ADDRESS_OUTSIDE_PROVIDER_RADIUS' using errcode = '22023'; end if;
    ...
    select q.provider_id, p.neighborhood, p.city into v_quote from public.quotes q join public.providers p on p.id = q.provider_id where q.id = p_quote_id and q.student_id = auth.uid();
