-- Fix the public driving-school onboarding validation contract.
-- PostgreSQL receives standard-conforming strings, so the previous double
-- escaped regular expressions (for example, \\d) matched a literal backslash
-- instead of digit characters and rejected valid phone numbers.
...
DECLARE
  ...
  v_expected integer;
BEGIN
  IF length(v_digits) <> 14 OR v_digits = repeat(left(v_digits, 1), 14) THEN
    RETURN false;
  END IF;
  ...
  IF substring(v_digits, 13, 1)::integer <> v_expected THEN
    RETURN false;
  END IF;
...
BEGIN
  ...
  END IF;
  IF v_phone !~ '^[0-9]{10,11}$' THEN RAISE EXCEPTION 'SCHOOL_PHONE_INVALID' USING ERRCODE = '22023'; END IF;
  IF v_email = '' OR v_email !~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$' THEN
    RAISE EXCEPTION 'SCHOOL_EMAIL_INVALID' USING ERRCODE = '22023';
  END IF;
  IF v_postal_code !~ '^[0-9]{8}$' OR v_city = '' OR v_state !~ '^[A-Z]{2}$' THEN
  ...
  ELSE
    INSERT INTO public.providers (
      ...
      p_address, p_latitude, p_longitude, 8
