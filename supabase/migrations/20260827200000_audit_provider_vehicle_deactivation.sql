-- Provider vehicle deactivation must be visible in the Admin audit trail.
...
DECLARE
  v public.vehicles;
  v_previous_status public.vehicle_status;
BEGIN
  ...
  IF NOT FOUND OR NOT EXISTS (
    SELECT 1 FROM public.providers p WHERE p.id=v.provider_id AND p.user_id=auth.uid()
  ) THEN
    RAISE EXCEPTION 'VEHICLE_ACCESS_DENIED' USING ERRCODE='42501';
  END IF;
  IF v.status <> 'ACTIVE' THEN
    RAISE EXCEPTION 'VEHICLE_DEACTIVATION_INVALID' USING ERRCODE='22023';
  END IF;
  ...
  INSERT INTO public.audit_logs(
    ...
    gen_random_uuid(), auth.uid(), 'VEHICLE_DEACTIVATED', 'Vehicle', p_vehicle_id::text,
