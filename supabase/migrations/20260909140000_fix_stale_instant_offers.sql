-- Keep Aula Agora availability preview and offer dispatch aligned.
-- An expired PENDING offer can otherwise block the unique instructor offer
-- constraint while the preview still counts that instructor as available.
...
BEGIN
  ...
  -- Clean up expired offers from every request before the unique instructor
  -- offer constraint is checked by the INSERT below.
  UPDATE public.instant_lesson_offers
