-- TASK-093 — Admin-configurable Aula Agora limits
-- DEV only. Keeps the existing agenda/booking rules unchanged.
...
VALUES (
  ...
  jsonb_build_object('max_eta_minutes', 30, 'offer_expiration_seconds', 15),
  'Parâmetros operacionais das ofertas Aula Agora.'
)
ON CONFLICT (key) DO UPDATE SET
  value = platform_configurations.value || jsonb_build_object(
    'max_eta_minutes', COALESCE(platform_configurations.value->'max_eta_minutes', '30'::jsonb),
    'offer_expiration_seconds', COALESCE(platform_configurations.value->'offer_expiration_seconds', '15'::jsonb)
  );
...
LANGUAGE SQL STABLE SECURITY DEFINER SET search_path = public, pg_temp AS $$
  SELECT
    GREATEST(1, LEAST(120, COALESCE((value->>'max_eta_minutes')::INTEGER, 30))),
...
BEGIN
  ...
  VALUES (
    'instant_lesson_settings',
    jsonb_build_object('max_eta_minutes', p_max_eta_minutes, 'offer_expiration_seconds', p_offer_expiration_seconds),
    'Parâmetros operacionais das ofertas Aula Agora.',
    v_uid,
    NOW()
  )
...
REVOKE ALL ON FUNCTION public.update_admin_instant_lesson_config(INTEGER, INTEGER) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_instant_lesson_platform_config() TO authenticated;
GRANT EXECUTE ON FUNCTION public.update_admin_instant_lesson_config(INTEGER, INTEGER) TO authenticated;
