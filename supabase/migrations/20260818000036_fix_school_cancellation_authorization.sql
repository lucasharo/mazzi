-- ============================================================================
-- MAZZI MIGRATION 20260818000036: FIX SCHOOL CANCELLATION AUTHORIZATION & GUARDS
-- ============================================================================
...
BEGIN
  ...
  -- 5. Strict Status Whitelist (ONLY CONFIRMED is cancelable via commercial RPC)
  ...
  -- 6. Past Lesson / Time Window Guard (scheduled_start_at > NOW() mandatory)
  ...
  -- 7. Role & Ownership Authorization Matrix
  ...
  ELSIF v_user_role IN ('SCHOOL_ADMIN', 'DRIVING_SCHOOL') THEN
    ...
    ELSE
      -- Check canonical helper or driving_school_staff relation
      SELECT EXISTS (
      ...
      -- Also check if provider itself has user_id matching auth.uid()
